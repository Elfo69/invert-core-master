...
//   } else {
 ...
 //	std::cout << "Modify this line into your invertX configuration file as is:  " << std::endl;
 //	std::cout << "const char GENESIS_COINBASE_TX_HEX[] = \"" << tx_hex << "\";" << std::endl;
//   }
//   return;
// }
...
void renameDataDir() {
  std::string invertXDir = Tools::getDefaultDataDirectory();
  boost::filesystem::path invertXDirPath(invertXDir);
  if (boost::filesystem::exists(invertXDirPath)) {
    return;
  }

  std::string dataDirPrefix = invertXDir.substr(0, invertXDir.size() + 1 - sizeof(CRYPTONOTE_NAME));
  boost::filesystem::path cediDirPath(dataDirPrefix + "BXC");

  if (boost::filesystem::exists(cediDirPath)) {
    boost::filesystem::rename(cediDirPath, invertXDirPath);
  } else {
    boost::filesystem::path BcediDirPath(dataDirPrefix + "Bcedi");
    if (boost::filesystem::exists(boost::filesystem::path(BcediDirPath))) {
		boost::filesystem::rename(BcediDirPath, invertXDirPath);
    }
  }
}
...
int main(int argc, char* argv[])
{
  ...
  try {
   ...
   if (!r)
   {
    ...
    logger(INFO, BRIGHT_YELLOW) << "invert v" << PROJECT_VERSION_LONG;
