...
bool processServerAliasResponse(const std::string& s, std::string& address) {
  ...
  // Courtesy of Monero Project
		// make sure the txt record has "oa1:inv" and find it
		auto pos = s.find("oa1:inv");
		if (pos == std::string::npos)
			return false;
...
simple_wallet::simple_wallet(System::Dispatcher& dispatcher, const CryptoNote::Currency& currency, Logging::LoggerManager& log) :
  ...
  m_consoleHandler.setHandler("save_keys", boost::bind(&simple_wallet::save_keys_to_file, this, boost::arg<1>()), "Save wallet private keys to \"invert_keys_backup.txt\"");
}
...
bool simple_wallet::new_wallet(const std::string &wallet_file, const std::string& password) {
  ...
  try {
    ...
    logger(INFO, BRIGHT_GREEN) << "invertWallet is an open-source, client-side, free wallet which allow you to send and receive inv instantly on the blockchain. You are  in control of your funds & your keys. When you generate a new wallet, login, send, receive or deposit $inv everything happens locally. Your seed is never transmitted, received or stored. That's why its imperative to write, print or save your seed somewhere safe. The backup of keys is your responsibility. If you lose your seed, your account can not be recovered. The invert Team doesn't take any responsibility for lost funds due to nonexistent/missing/lost private keys." << std::endl << std::endl;
...
bool simple_wallet::get_reserve_proof(const std::vector<std::string> &args)
{
	...
	try {
		...
		const std::string filename = "reserve_proof_" + args[0] + "_inv.txt";
...
bool simple_wallet::export_keys(const std::vector<std::string>& args/* = std::vector<std::string>()*/) {
  ...
  logger(INFO, BRIGHT_GREEN) << std::endl << "invertWallet is an open-source, client-side, free wallet which allow you to send and receive inv instantly on the blockchain. You are  in control of your funds & your keys. When you generate a new wallet, login, send, receive or deposit $inv everything happens locally. Your seed is never transmitted, received or stored. That's why its imperative to write, print or save your seed somewhere safe. The backup of keys is your responsibility. If you lose your seed, your account can not be recovered. The invert Team doesn't take any responsibility for lost funds due to nonexistent/missing/lost private keys." << std::endl << std::endl;
...
{
  std::ofstream backup_file("invert_keys_backup.txt");
  AccountKeys keys;
  m_wallet->getAccountKeys(keys);

  std::string priv_key = "\t\tinvert Keys Backup\n\n";
  ...
  logger(INFO, BRIGHT_GREEN) << "Wallet keys have been saved to the current folder where \"invertwallet\" is located.";
...
int main(int argc, char* argv[]) {
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]() {
    ...
    if (command_line::get_arg(vm, command_line::arg_help)) {
      ...
      std::cout << "invert Wallet v" << PROJECT_VERSION_LONG << std::endl;
      std::cout << "Usage: invertwallet [--wallet-file=<file>|--generate-new-wallet=<file>] [--daemon-address=<host>:<port>] [<COMMAND>]";
  ...
  logger(INFO, BRIGHT_GREEN) << "invert Wallet v" << PROJECT_VERSION_LONG;
