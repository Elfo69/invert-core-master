...
void cn_invert_slow_hash_v0(cn_context &context, const void *data, size_t length, Hash &hash) {
	if(hw_check_aes())
		cryptonight_hash<false, CRYPTONIGHT_invert>(data, length, reinterpret_cast<char *>(&hash), context);
	else
		cryptonight_hash<true, CRYPTONIGHT_invert>(data, length, reinterpret_cast<char *>(&hash), context);
}
